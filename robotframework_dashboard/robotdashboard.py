...
class RobotDashboard:
            ...
            if outputs:
                for output in outputs:
                    try:
                        ...
                        console += self._print_console(f"  ERROR: Could not process output XML '{basename(output_path)}', error: {error}")
                        ...
                        for file in files:
                            try:
                                if "output" in file and ".xml" in file:
                                    start = time()
                                    console += self._print_console(f"  Processing output XML '{join(subdir, file)}'")
                                ...
                                console += self._print_console(f"  ERROR: Could not process found output file '{file}', error: {error}")
                else:
                    console += self._print_console(f"  ERROR: Could not process output folder '{output_folder_path}', error: the path does not exist!")
        else:
            console += self._print_console(f" 2. Processing output XML(s)\n  skipping step")
        console += self._print_console("======================================================================================")
        database.close_database()
    ...
    def create_dashboard(self):
        ...
        if self.generate_dashboard:
            ...
            console += self._print_console(" 5. Creating dashboard HTML\n  skipping step")
        return console
